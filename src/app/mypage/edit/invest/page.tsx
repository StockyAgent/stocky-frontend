...
type PersonaType = "investor" | "trader";

export default function EditInvestPage() {
  const router = useRouter();
  // 현재 저장된 투자 성향 (트레이더로 가정)
  const currentPersona: PersonaType = "trader";
  ...
  return (
    <div className="relative flex min-h-dvh flex-col bg-[#f1faee]">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 bg-linear-to-t from-[#f1faee] via-[#f1faee] to-transparent pb-8 pt-5 flex justify-center px-5">
        <PrimaryButton onClick={handleSave} disabled={!hasChanged}>
          수정
        </PrimaryButton>
      </div>
    </div>
