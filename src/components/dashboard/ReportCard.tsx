...
export default function ReportCard({ type, date, title, description, onClick }: ReportCardProps) {
  ...
  return (
    <button
      onClick={onClick}
      className="flex w-full items-center gap-4 rounded-[16px] border border-[#e1e1e1] bg-white p-[21px] text-left cursor-pointer"
    >
      <div className="flex flex-1 flex-col gap-[6px]">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] font-medium tracking-[-0.25px] text-[#457b9d]" style={{ fontFamily: "Inter, sans-serif" }}>
            {date}
          </span>
        </div>
        {/* Title */}
        <h4 className="truncate text-[15px] font-bold text-[#1d3557]">{title}</h4>
        {/* Description */}
        <p className="truncate text-[12px] font-[350] text-[#457b9d]">{description}</p>
      </div>
      <Image
